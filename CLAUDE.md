...
**What:** Portfolio-project monorepo, grown step by step. Backend has its first business module (`Environment` CRUD, package-by-feature under `environment/`, cross-cutting concerns under `common/`) with Flyway-managed schema migrations. Frontend is still a generated skeleton beyond the backend health check display. `infra/k8s/`, `infra/helm/` are empty placeholders. `docs/` holds [PROJECT_GUIDE.md](docs/PROJECT_GUIDE.md) (goals/roadmap), [ARCHITECTURE.md](docs/ARCHITECTURE.md) (current stack/layout), [DEVELOPMENT.md](docs/DEVELOPMENT.md) (commands/verification), and `DECISIONS/` (ADRs, currently 0001-0016). CI is in place via GitHub Actions (`.github/workflows/ci.yml`, see ADR-0011) — backend tests + frontend lint/build on every push/PR to `main`; branch protection not enabled yet. Container images are hardened (non-root prod backend, pinned base-image versions, consistent healthchecks across all services — `db`, `backend`, `frontend`, `prometheus`, `grafana`, see ADR-0012). Observability foundation (roadmap step 6) is done: the backend exposes `/actuator/prometheus` (Micrometer), scraped by a `prometheus` service (ADR-0013), visualized via a `grafana` service connected to Prometheus as a data source (ADR-0015), with a basic technical dashboard covering RED (request rate/errors/latency) and USE (JVM heap/CPU, Hikari pool) panels (ADR-0016, dashboard JSON backed up at `infra/grafana/basic-technical.json`).

**Why:** Repo is grown step by step with production-like practices. Roadmap: API contract + frontend/backend integration (done) → first domain module (done — `Environment` CRUD, ADRs 0005-0010) → CI (done — GitHub Actions, ADR-0011) → containerization (done — hardening, ADR-0012) → observability (done — Actuator/Micrometer + Prometheus + Grafana + basic dashboard, ADR-0013/ADR-0015/ADR-0016) → security foundation (next) → Kubernetes.
